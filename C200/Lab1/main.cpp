	...
	//предусмотрите инициализацию переменных класса, например, нулем
{
  ...
  //2б. Объявите и определите в классе Rect метод InflateRect();
	...
	/*
		...
		Спецификатор доступа private означает, что члены класса (переменные или функции);этим модификатором доступа 
...
/*
	...
	// Rect BoundingRect(Rect, Rect);оторая принимает два объекта класса Rect по значению и
...
/*
	...
	//конструкторе (конструкторах);деструкторе. Определите: когда для
	//каждого из объектов вызывается конструктор, а когда - деструктор?
*/
	{
		Rect r1;               // Вызывается конструктор по умолчанию для r1
		Rect* pR = new Rect(1, 2, 1, 2);  // Вызывается конструктор с параметрами для pR
		{
...
/* 
	...
	// Посредством конструктора (конструкторов);деструктора обеспечьте
...
*/
	{
		...
		//Создайте метод GetString();оторый обеспечит доступ к хранящейся строке.
		//С помощью cout и метода GetString();спечатайте строку объекта str1
	    ...
	    // str1.GetString(); = 'W'; // ошибка, так как GetString();звращает const char*
...
*/
	...
	//времени содержится спирт (96%); другой – вода.
