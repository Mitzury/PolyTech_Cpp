...
// функтор(функциональный объект), обеспечивающий преобразование символа 
// строки в нижний регистр.
// Функторы в C++ - это объекты, которые можно вызывать так, как если бы они были функциями. 
// В данном случае этот класс перегружает оператор () и принимает символ в качестве параметра. 
// Он использует функцию tolower() из стандартной библиотеки C, чтобы преобразовать символ в нижний регистр. 
// Затем функция возвращает преобразованный символ.
class ToLower {
public:
